...
import { getUserById } from "@/actions/read/get-users"; // Custom actions for fetching user data
...
export const {
      ...
      callbacks: {
            ...
            async session({ token, session }: { token: any, session: any }) {
                  ...
                  if (session.user) {
                        session.user.firstName = token.firstName; // Add user name to session
                  }

                  if (session.user) {
                        session.user.lastName = token.lastName; // Add user name to session
                  }

                  if (session.user) {
                        session.user.phone = token.phone; // Add user name to session
                  }
            ...
            async jwt({ token }: { token: any }) {
                  ...
                  if (existingUser) {
                        ...
                        token.firstName = existingUser.firstName; // Add username to token
                        token.lastName = existingUser.lastName; // Add username to token
